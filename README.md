
## How to Clone and Contribute

Clone the project

```bash
  git clone https://github.com/wardislit/waste-mngmt-system.git
```

Go to the project directory

```bash
  cd .../waste-mngmt-system
```

Install dependencies if ever ([npm](https://www.npmjs.com/))

```bash
  npm install
```

## Commit and Push (Pull Requests):

- Watch [this](https://www.youtube.com/watch?v=jRLGobWwA3Y).

## Generating Tokens Tutorial

When creating personal tokens, go to top-right and click your profile picture and set-up the following:

- Go to Settings > Developer Settings > Personal access tokens

- Go to Tokens (Classic) and Generate new token

- Choose the classic for general use

- Fill up the important and make sure you click the Repository Access > All Repositories

- Then generate and done.

